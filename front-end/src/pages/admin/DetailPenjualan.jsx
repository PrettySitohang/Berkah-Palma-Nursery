...
const DetailPenjualan = () => {
  ...
  // Buat fungsi untuk kembali ke halaman Kelola Pesanan
  const handleBackToPesanan = () => {
    ...
    <div className="min-h-screen bg-slate-50 pb-28 font-sans" style={{ fontFamily: '"Poppins", sans-serif' }}>
      ...
      <div className="bg-[#2a4530] px-6 pt-10 pb-10 text-white rounded-b-[2rem] shadow-md relative z-10">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          ...
          <button 
            onClick={handleBackToPesanan}
            className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center hover:bg-white/20 transition-all active:scale-95 cursor-pointer"
          >
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-[280px] bg-white border-t border-slate-100 p-4 px-5 z-30">
        <div className="max-w-3xl mx-auto flex gap-3 w-full">
          ...
          <button 
            onClick={handleBackToPesanan}
            className="flex-1 py-3.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-[13px] hover:bg-slate-50 transition-colors active:scale-95 cursor-pointer"
          >
            TOLAK
          </button>
          ...
          <button 
            onClick={handleBackToPesanan}
            className="flex-[2] py-3.5 rounded-xl bg-[#2a4530] text-white font-bold text-[13px] hover:bg-[#1e3223] transition-colors shadow-lg shadow-[#2a4530]/20 active:scale-95 cursor-pointer"
          >
            KONFIRMASI PESANAN
          </button>
